import { NavContainer, NavItemStyles } from "../../../styles/nav_css";
import React, { useEffect, useState } from "react";
import { Global } from "@emotion/react";
import Link from "next/link";
import { useDispatch, useSelector } from "react-redux";
import { setIsLoginFalse, setIsLoginTrue } from "../../commons/store/store.js";
...
export default function NavBar() {
  ...
  useEffect(() => {
    ...
    !!token ? dispatch(setIsLoginTrue()) : dispatch(setIsLoginFalse());
  }, [loginFlag]);
    ...
    <div>
      <NavContainer>
        <div style={{ cursor: "pointer" }}>
          <Link href="/">
            <img src="img/logo.png" height={50}></img>
            {/* <svg
              id="logo-79"
              className="gradient"
              width="125"
        ...
        <div>
          {loginFlag ? (
            ...
            <ul
              id="navbar"
              className={sideMenuState == true ? "#navbar active" : "#navbar"}
            >
              {/* <li><Link href="/" className='active'>HOME</Link></li> */}
              <li>
                <Link href="/examine">진단하기</Link>
              </li>
              <li>
                <Link href="/mypage">등록하기</Link>
              </li>
              <li>
